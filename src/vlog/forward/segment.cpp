...
std::shared_ptr<Segment> Segment::sortByField(const uint8_t field) const {
...
std::shared_ptr<Segment> Segment::filter(const std::vector<Term_t> constants,
        const std::vector<size_t> posConstants) const
{
    auto itr = iterator();
    const size_t ncolumns = getNColumns();
    ...
    while (itr->hasNext())
    {
        ...
        {
            auto pos = posConstants[i];
            if (itr->get(pos) != constants[i])
            {
                ok = false;
                break;
            }
    ...
    for(size_t i = 0; i < ncolumns; ++i)
    {
        newColumns.push_back(writers[i].getColumn());
    }
    return std::shared_ptr<Segment>(new Segment(ncolumns, newColumns));
}
...
std::shared_ptr<const Segment> SegmentInserter::unique(
    ...
    if (seg->getNColumns() == 1) {
        ...
        const size_t ncolumns = seg->getNColumns();
        const size_t ncolumnsToCheck = nfields == -1 ? ncolumns : nfields;
        ...
        while (itr->hasNext()) {
            itr->next();
            bool unq = false;
            for (int i = 0; i < ncolumnsToCheck; ++i)
