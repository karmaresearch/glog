...
std::shared_ptr<const TGSegment> TGChase::retain(
        PredId_t p,
        std::shared_ptr<const TGSegment> newtuples) {
    if (!g.areNodesWithPredicate(p)) {
        return newtuples;
    }
    auto &nodeIdxs = g.getNodeIDsWithPredicate(p);
        ...
        if (!cacheRetain.count(p) || cacheRetain[p].nnodes < nodeIdxs.size()) {
            ...
            if (newtuples->getNColumns() == 1) {
                ...
                for(size_t i = cacheRetain[p].nnodes; i < nodeIdxs.size(); ++i) {
                    g.getNodeData(nodeIdxs[i])->appendTo(0, tuples);
                }
                ...
                for(size_t i = cacheRetain[p].nnodes; i < nodeIdxs.size(); ++i) {
                    g.getNodeData(nodeIdxs[i])->appendTo(0, 1, tuples);
                }
                std::sort(tuples.begin(), tuples.end());
...
bool TGChase::executeRule(GBRuleInput &node) {
    ...
    if (nonempty) {
        ...
        auto retainedTuples = retain(currentPredicate, derivations);
